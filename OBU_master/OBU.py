...
def processor(can_msg):
    ...
    if can_msg is not None:
        ...
        elif order_id == "brake_override" and data == 0:
            propulsion_override = 0

        elif order_id == "accel_pedal" and propulsion_override == 0:
            propulsion(data)
            print("accel = ",data)
            
        elif propulsion_override == 1:
            propulsion(0)
            print("accel override")
    ...
    try:
        with can.interface.Bus(channel='can0', bustype='socketcan', receive_own_messages=False) as bus:
            ...
            while True:
                ...
                processor(can_msg)
                braking_value=input("enter braking value")
                braking_value=int(braking_value)
                can_send(bus,"BRAKE","brake_set",braking_value)
                time.sleep(0.1)
